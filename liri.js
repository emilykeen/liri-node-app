...
if (command === "my-tweets") {
    ...
    if ((process.argv[3]) === undefined) {
        console.log('false');
        request("https://api.spotify.com/v1/artists/5ksRONqssB7BR161NTtJAm/top-tracks?country=US", function(error, response, body) {
             //Artist(s)
                console.log("Artist: "+ JSON.parse(body).tracks[1].artists[0].name);
                 // The song's name
                  console.log("Song: "+ JSON.parse(body).tracks[1].name);
                 // A preview link of the song from Spotify
                 console.log("Preview Link: "+ JSON.parse(body).tracks[1].preview_url);
                 // The album that the song is from
                 console.log("album: "+ JSON.parse(body).tracks[1].album.name);
            })
        }
            request("https://api.spotify.com/v1/search?q=" + song + "&type=track&limit=1", function(error, response, body) {

                // If there were no errors and the response code was 200 (i.e. the request was successful)...
                if (!error && response.statusCode === 200 && (process.argv[3]) !== undefined) {
